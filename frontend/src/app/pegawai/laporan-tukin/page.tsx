...
import { personalLaporanTukinAPI, type LaporanTukin, type LaporanTukinRequest, type LaporanTukinHistoriRequest, type PagedResponse } from '@/lib/api';
...
export default function LaporanTukinPage() {
  ...
  const fetchHistori = async () => {
    ...
    try {
      ...
      const result = await personalLaporanTukinAPI.getHistori(request);
      ...
      console.error('Error fetching histori laporan pribadi:', error);
      
      // Reset to empty state on error but don't show error toast
      // as the API already handles the error gracefully
      setHistori({
        content: [],
        totalElements: 0,
        totalPages: 0,
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await personalLaporanTukinAPI.generate(request);
      
      toast.success('Laporan Tukin berhasil dibuat!');
      
      // Redirect to detail page
      if (result.id) {
        router.push(`/pegawai/laporan-tukin/${result.id}`);
      } else {
  ...
  const handleViewDetail = (id: number) => {
    router.push(`/pegawai/laporan-tukin/${id}`);
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await personalLaporanTukinAPI.delete(deleteDialog.laporan.id);
  ...
  return (
    <div className="container mx-auto py-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold">Laporan Tunjangan Kinerja Pribadi</h1>
          <p className="text-gray-600 mt-2">Generate dan kelola laporan tunjangan kinerja pribadi Anda</p>
        </div>
                ...
                <TableRow>
                  <TableCell colSpan={8} className="text-center py-8">
                    <div className="flex flex-col items-center space-y-2">
                      ...
                      <p className="text-gray-600">Belum ada laporan pribadi yang digenerate</p>
                      <p className="text-sm text-gray-500">Klik "Generate Laporan" untuk membuat laporan pribadi Anda</p>
                    </div>
                  </TableCell>
                </TableRow>
              ) : (
